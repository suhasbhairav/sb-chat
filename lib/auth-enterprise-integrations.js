...
function buildMicrosoftSSOProvider() {
  if (!boolEnv("BATUK_MICROSOFT_SSO_ENABLED", false)) return null;

  const clientId = process.env.BATUK_MICROSOFT_CLIENT_ID;
  const clientSecret = process.env.BATUK_MICROSOFT_CLIENT_SECRET;
  const tenantId = process.env.BATUK_MICROSOFT_TENANT_ID;
  const domain = process.env.BATUK_MICROSOFT_DOMAIN;

  if (!clientId || !clientSecret || !tenantId || !domain) {
    console.warn(
      "BATUK_MICROSOFT_SSO_ENABLED is true, but BATUK_MICROSOFT_CLIENT_ID, BATUK_MICROSOFT_CLIENT_SECRET, BATUK_MICROSOFT_TENANT_ID, or BATUK_MICROSOFT_DOMAIN is missing.",
    );
    return null;
  }
  ...
  return {
    providerId: process.env.BATUK_MICROSOFT_PROVIDER_ID || "microsoft-entra",
    issuer,
    domain,
    oidcConfig: {
      issuer,
      ...
      pkce: true,
      scopes: commaEnv("BATUK_MICROSOFT_SCOPES", ["openid", "profile", "email", "offline_access"]),
      mapping: {
        id: "sub",
        email: "email",
...
function defaultSSOProviders() {
  return [
    ...jsonEnv("BETTER_AUTH_DEFAULT_SSO", []),
    buildMicrosoftSSOProvider(),
  ].filter(Boolean);
}
