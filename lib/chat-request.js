...
export function validateChatPayload(payload) {
  ...
  if (payload.provider === "orcarouter" && !resolveServerApiKey(payload.provider, payload.apiKey)) {
    return "OrcaRouter needs an API key. Add ORCAROUTER_API_KEY to .env or enter an OrcaRouter key in Settings.";
  }
  if (payload.provider === "velona" && !resolveServerApiKey(payload.provider, payload.apiKey)) {
    return "Velona needs an API key. Add VELONA_API_KEY to .env or enter a Velona key in Settings.";
  }
  ...
  if (payload.provider === "edenai" && !resolveServerApiKey(payload.provider, payload.apiKey)) {
    return "EdenAI needs an API key. Add EDENAI_API_KEY to .env or enter an EdenAI key in Settings.";
  }
  if (payload.provider === "deepinfra" && !resolveServerApiKey(payload.provider, payload.apiKey)) {
    return "DeepInfra needs an API key. Add DEEPINFRA_API_KEY to .env or enter a DeepInfra token in Settings.";
  }
  ...
  if (payload.provider === "bedrock" && !String(process.env.AWS_BEDROCK_REGION || process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION || payload.baseUrl || "").trim()) {
    return "AWS Bedrock needs an AWS region. Set AWS_BEDROCK_REGION, AWS_REGION, or enter a region in Base URL.";
  }
  return null;
}
...
export function resolveServerApiKey(provider, apiKey) {
  ...
  if (provider === "orcarouter") {
    return process.env.ORCAROUTER_API_KEY || process.env.ORCA_ROUTER_API_KEY || "";
  }

  if (provider === "velona") {
    return process.env.VELONA_API_KEY || "";
  }
  ...
  if (provider === "edenai") {
    return process.env.EDENAI_API_KEY || process.env.EDEN_AI_API_KEY || "";
  }

  if (provider === "deepinfra") {
    return process.env.DEEPINFRA_API_KEY || process.env.DEEPINFRA_TOKEN || "";
  }
