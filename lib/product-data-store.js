...
export function resolveProductDataScope(session) {
  ...
  return {
    organizationId: String(activeOrganizationId || "personal"),
...
export function resolvePersonalProductDataScope(session) {
  return resolveProductDataScope(session);
}

export function resolveWorkspaceProductDataScope(session, workspaceId) {
  const personal = resolveProductDataScope(session);
  return {
    organizationId: personal.organizationId,
    userId: `workspace:${String(workspaceId || "").slice(0, 140)}`,
    scopeType: "workspace",
    workspaceId: String(workspaceId || "").slice(0, 140),
  };
}
...
function cleanScope(scope = {}) {
  return {
    ...
    scopeType: scope.scopeType === "workspace" ? "workspace" : "personal",
    workspaceId: scope.workspaceId ? String(scope.workspaceId).slice(0, 160) : null,
  };
}
