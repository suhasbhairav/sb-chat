...
const EMBEDDING_PROVIDERS = new Set(["local", "openai", "ollama", "llamaindex-openai", "llamaindex-ollama"]);
...
function normalizeOllamaBaseUrl(baseUrl) {
  return normalizeBaseUrl(baseUrl || process.env.OLLAMA_BASE_URL || process.env.BATUK_OLLAMA_BASE_URL || "http://localhost:11434");
}

function assertEmbeddingBatch(texts, embeddings, providerName) {
  if (!Array.isArray(embeddings) || embeddings.length !== texts.length) {
    throw new Error(`${providerName} returned ${Array.isArray(embeddings) ? embeddings.length : 0} embeddings for ${texts.length} inputs.`);
  }

  const dimensions = embeddings[0]?.length;
  if (!dimensions) {
    throw new Error(`${providerName} returned empty embeddings.`);
  }

  for (const embedding of embeddings) {
    if (!Array.isArray(embedding) || embedding.length !== dimensions) {
      throw new Error(`${providerName} returned embeddings with inconsistent dimensions.`);
    }
  }
...
async function createOllamaEmbeddings(texts, { baseUrl, ollamaBaseUrl, model = DEFAULT_OLLAMA_EMBEDDING_MODEL } = {}) {
  const response = await fetch(`${normalizeOllamaBaseUrl(ollamaBaseUrl || baseUrl)}/api/embed`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    throw new Error(data.error || `Ollama embeddings failed with status ${response.status}. Pull the model with: ollama pull ${model}`);
  }

  return assertEmbeddingBatch(texts, data.embeddings, "Ollama");
}
...
async function createLlamaIndexOllamaEmbeddings(
  ...
  Settings.embedModel = new OllamaEmbedding({
    model,
    config: {
      host: normalizeOllamaBaseUrl(ollamaBaseUrl || baseUrl),
    },
  });
...
export function normalizeEmbeddingSettings(settings = {}) {
  ...
  if (embeddingProvider === "ollama") {
    return { embeddingProvider, embeddingModel: settings.embeddingModel || envModel || DEFAULT_OLLAMA_EMBEDDING_MODEL };
  }
...
export async function createEmbeddings(texts, options = {}) {
  ...
  if (normalized.embeddingProvider === "ollama") {
    return createOllamaEmbeddings(texts, embeddingOptions);
  }
