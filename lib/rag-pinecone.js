...
export async function queryPineconeChunks(queryEmbedding, settings = {}) {
  ...
  const result = await index.query({
    ...
    ...(settings.scopeType
      ? {
          filter: {
            scopeType: { $eq: String(settings.scopeType) },
            organizationId: { $eq: String(settings.organizationId || "") },
            userId: { $eq: String(settings.userId || "") },
          },
        }
      : {}),
  });
  ...
  return (result.matches || [])
    .map((match) => {
      ...
      return {
        ...
        elementType: metadata.elementType || null,
        sourceElementIds: metadata.sourceElementIds ? JSON.parse(metadata.sourceElementIds) : [],
      };
    })
