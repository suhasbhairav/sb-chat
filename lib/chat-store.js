...
function getScopedStorePath() {
  ...
  return path.join(DATA_DIR, "scoped-chat", safeScopeSegment(scope.organizationId), `${safeScopeSegment(scope.userId)}.json`);
}
...
function cleanWorkspace(workspace) {
  ...
  return {
    ...
    scope: workspace.scope === "workspace" ? "workspace" : "personal",
    ownerId: workspace.ownerId ? String(workspace.ownerId).slice(0, 120) : null,
    members: Array.isArray(workspace.members) ? workspace.members.map((member) => String(member).slice(0, 120)).filter(Boolean) : [],
    memberDetails: Array.isArray(workspace.memberDetails) ? workspace.memberDetails.slice(0, 500) : [],
    ragEnabled: Boolean(workspace.ragEnabled),
...
function defaultStore() {
  ...
  return {
    version: 1,
    workspaces: [{ id: workspaceId, name: "Personal", scope: "personal", ownerId: null, members: [], memberDetails: [], ragEnabled: true, createdAt, updatedAt: createdAt }],
    folders: [
...
async function ensureStoreFile() {
  ...
  try {
    await readFile(getScopedStorePath(), "utf8");
  } catch {
...
export async function readChatStore() {
  if (isSqlProductDataStoreEnabled()) {
    return readSqlDomainStore("chat-library", defaultStore());
  }
...
export async function writeChatStore(store) {
  if (isSqlProductDataStoreEnabled()) {
    return writeSqlDomainStore("chat-library", store);
  }
...
export async function createWorkspace(name, { ownerId = null, members = [], memberDetails = [], shared = false, ragEnabled = true } = {}) {
  ...
  const workspace = {
    ...
    ownerId: ownerId ? String(ownerId).slice(0, 120) : null,
    members: Array.isArray(members) ? members.map((member) => String(member).slice(0, 120)).filter(Boolean) : [],
    memberDetails: Array.isArray(memberDetails) ? memberDetails.slice(0, 500) : [],
