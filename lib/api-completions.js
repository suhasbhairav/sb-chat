...
export async function handleChatCompletionRequest({
  ...
  if (!auth) {
    await recordAuditEvent?.({
      category: "access",
      action: "api.chat.authentication_failed",
      outcome: "denied",
      statusCode: 401,
      ...auditContext,
    });
  ...
  if (!messages.length) {
    await recordAuditEvent?.({
      category: "model",
      action: "api.chat.validation_failed",
      outcome: "failure",
      actor: auth.key,
      statusCode: 400,
      metadata: { reason: "messages is required", model: payload.model },
      ...auditContext,
    });
    return {
      status: 400,
  ...
  if (!route) {
    await recordAuditEvent?.({
      category: "model",
      action: "api.chat.model_not_found",
      outcome: "denied",
      actor: auth.key,
      statusCode: 404,
      metadata: { requestedModel: payload.model },
      ...auditContext,
    });
    return {
      status: 404,
  ...
  await recordAuditEvent?.({
    category: "model",
    action: "api.chat.completion.finished",
    outcome: "success",
    actor: { id: auth.key.userId, email: auth.key.userEmail, role: "api_key" },
    target: { type: "api_model", id: route.id },
    metadata: {
      apiKeyId: auth.key.id,
      ...
      promptDigest: messages.map((message, index) => ({
        index,
        role: message.role,
        length: message.content.length,
      })),
      resultLength: String(result.message || "").length,
      usage,
    },
    ...auditContext,
  });
