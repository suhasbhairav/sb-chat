...
function sanitizeAttachments(attachments = [], { includeAttachmentData = false } = {}) {
  return attachments
    ...
    .map((attachment) => {
      ...
      if (attachment.text) {
        clean.text = String(attachment.text).slice(0, includeAttachmentData ? 30000 : 12000);
      }

      if (includeAttachmentData && attachment.kind === "image" && attachment.data) {
        clean.data = String(attachment.data);
      }
...
export function sanitizeMessages(messages, options = {}) {
  return normalizeMessages(messages)
    ...
    .map(({ role, content, attachments }) => {
      const cleanAttachments = sanitizeAttachments(attachments, options);
      return {
        role,
        content,
